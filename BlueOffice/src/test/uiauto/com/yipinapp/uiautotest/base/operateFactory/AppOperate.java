package com.yipinapp.uiautotest.base.operateFactory;
...
public abstract class AppOperate {
	...
	/**
	 * wait for the text present in timeout setting 在指定时间内等待，直到文本出现在页面上。
	 *
	...
	/**
	 * wait for the text present in timeout setting 在指定时间内等待，直到文本消失在页面上。
	 *
	 * @param timeoutInSeconds
	 *            设置等待时间,单位:秒.
	 * @param TargetText
	...
	/**
	 * override original method 模拟点击,在原生方法执行时添加日志。
	 *
	 * @param element
	...
	/**
	 * override original method 模拟输入,在原生方法执行时添加日志。
	 *
	 * @param element
	 *            the element to be found.
	 * @param LogText
	 *            input log text.
	 * @param charSequences
	 *            sendKeys content, support for multiple strings.
	 */
	...
	/**
	 * override original method 模拟清除,在原生方法执行时添加日志。
	 *
	 * @param element
	 *            the element to be found.
	 * @param LogText
	...
	/**
	 * override original method 获取文本,在原生方法执行时添加日志。
	 *
	 * @param element
	 *            the element to be found.
	 * @param LogText
	...
	/**
	 * override original method 获取属性,在原生方法执行时添加日志。
	 *
	 * @param element
	 *            the element to be found.
	 * @param LogText
	...
	/**
	 * override original method 滚动到目标文本的位置,在原生方法执行时添加日志。
	 *
	 * @param targetText
	 *            the target text to be scrollToExact.
	 * @param LogText
	 *            input log text.
	 */
	...
	/**
	 * override original method 模拟手指在目标元素上持续轻触一段时间,在原生方法执行时添加日志。
	 *
	 * @param fingers
	...
	/**
	 * override original method 锁屏指定时间后解锁,在原生方法执行时添加日志。
	 *
	 * @param LogText
	 *            input log text.
	 * @param timeoutInSeconds
	 *            time for waiting, unit: second.
	 */
	...
	public void swipe_elementA_TO_elementB(WebElement elementA, WebElement elementB, String LogText) {
		Log.logStep("[" + LogText + "] ");
		Log.logInfo("[" + LogText + "] ");
		new TouchAction(((AppiumDriver<?>) driver)).longPress(elementA).moveTo(elementB).release().perform();
	}
	...
	public void GesturePWD(List<WebElement> webElements) {
		...
		for (int i = 1; i < webElements.size(); i++) {
			...
			touchAction.moveTo(webElements.get(i).getLocation().x - webElements.get(i - 1).getLocation().x,
					webElements.get(i).getLocation().y - webElements.get(i - 1).getLocation().y);
		}
		touchAction.release();
		touchAction.perform();
	}
	 ...
	 * @author quqing
	 */
	public void swipeToDown(int during, int startXDenominator, int startYDenominator, int endXDenominator,
			int endYDenominator) {
		int width = ((AppiumDriver<?>) driver).manage().window().getSize().width;
		int height = ((AppiumDriver<?>) driver).manage().window().getSize().height;
		((AppiumDriver<?>) driver).swipe(width / startXDenominator, height / startYDenominator, width / endXDenominator,
				height / endYDenominator, during);
	}
	...
	/**
	 * Description: set element locate timeout. 内容描述：设置对象查找超时时间.
	 *
	 * @param time
	...
	/**
	 * wait for the element clickable in timeout setting 在指定时间内等待，直到对象能够被点击。
	 *
	 * @param by
	...
	/**
	 * judge if the element is present in specified seconds 在指定的时间内判断指定的对象是否存在。
	 *
	 * @param by
	 *            the element locator By
	 * @param duration
	...
	public boolean elementExists(int duration, WebElement webElement) {
		...
		while (!exists && ((System.currentTimeMillis() - start) <= duration * 1000)) {
			try {
				exists = (null != webElement.getLocation()) ? true : false;
			} catch (NoSuchElementException ne) {
				exists = false;
			}
		}
	 ...
	 * @param during
	 */
	public void clickListItem(WebElement listView,List<WebElement> elements,String itemName,int during){
		int x = listView.getLocation().getX();
		int y = listView.getLocation().getY();
		int hight = listView.getSize().getHeight();
		int wigh = listView.getSize().getWidth();
		boolean isStart = true;
		while (isStart) {
			List<WebElement> list = elements;
			for (WebElement element : list) {
				System.out.println(element.getAttribute("text"));
				if (element.getAttribute("text").equals(itemName)) {
					element.click();
					isStart = false;
					break;
				}
			...
			if(isStart = true){
				((AppiumDriver<?>) driver).swipe(x + wigh / 2, y + hight - 1, x + wigh / 2, y, during);
			}
		}
	}
	 ...
	 * @param during
	 */
	public void clickListItem(WebElement listView,List<WebElement> elements,String[] itemsName,int during){
		int x = listView.getLocation().getX();
		int y = listView.getLocation().getY();
		int hight = listView.getSize().getHeight();
		int wigh = listView.getSize().getWidth();
		...
		while (isStart) {
			...
			for (WebElement element : list) {
				System.out.println(element.getAttribute("text"));
				for(String iteamName:itemsName){
				if (element.getAttribute("text").equals(iteamName)) {
					element.click();
					isStart = false;
					break;
				}
